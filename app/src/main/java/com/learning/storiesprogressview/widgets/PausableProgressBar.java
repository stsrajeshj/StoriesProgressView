...
final class PausableProgressBar extends FrameLayout {
    ...
    public PausableProgressBar(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0, 0, 0, 0);
    }

    public PausableProgressBar(@NonNull Context context, @ColorInt int backProgress, @ColorInt int frontProgress, @ColorInt int maxProgress) {
        this(context, null, 0, backProgress, frontProgress, maxProgress);
    }

    public PausableProgressBar(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr, @ColorInt int backProgress, @ColorInt int frontProgress, @ColorInt int maxProgress) {

        super(context, attrs, defStyleAttr);

        initColors(backProgress, frontProgress, maxProgress);
        ...
        backProgressView.setBackgroundColor(this.backProgress);
        frontProgressView.setBackgroundColor(this.frontProgress);
        maxProgressView.setBackgroundColor(this.maxProgress);
    }

    private void initColors(int backProgress, int frontProgress, int maxProgress) {
        this.backProgress = backProgress == -1 ? ContextCompat.getColor(getContext(), R.color.defaultBackProgress) : backProgress;
        this.frontProgress = frontProgress == -1 ? ContextCompat.getColor(getContext(), R.color.defaultFrontProgress) : frontProgress;
        this.maxProgress = maxProgress == -1 ? ContextCompat.getColor(getContext(), R.color.defaultMaxProgress) : maxProgress;
    }
    ...
    void setMinWithoutCallback() {
        maxProgressView.setBackgroundColor(backProgress);
    ...
    void setMaxWithoutCallback() {
        maxProgressView.setBackgroundColor(maxProgress);
    ...
    private void finishProgress(boolean isMax) {
        if (isMax) maxProgressView.setBackgroundColor(maxProgress);
